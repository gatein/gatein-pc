...
public class ControllerRequestFactory
{
   ...
   public ControllerRequest decode(Map<String, String[]> queryParameterMap, Map<String, String[]> formParameterMap)
   {
      ...
      if (context != null)
      {
         byte[] bytes = Base64.decode(context, EncodingOption.USEURLSAFEENCODING);
      ...
      else
      {
         ...
         {
            byte[] bytes = Base64.decode(queryParameters.get(ControllerRequestParameterNames.PUBLIC_NAVIGATIONAL_STATE_CHANGES), EncodingOption.USEURLSAFEENCODING);
